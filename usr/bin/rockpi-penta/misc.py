...
def get_blk():
    conf["disk"] = [
        x for x in check_output(cmds["blk"]).strip().split("\n") if x.startswith("sd")
    ]
...
def read_conf():
    ...
    try:
        ...
        conf["fan"]["silentmaxlv"] = float(
            cfg.get("fan", "silentmaxlv", fallback="0.4").replace("%", "e-2")
        )
        conf["fan"]["silentstart"] = datetime.datetime.strptime(
            cfg.get("fan", "silentstart", fallback="22:00"), "%H:%M"
        ).time()
        conf["fan"]["silentend"] = datetime.datetime.strptime(
            cfg.get("fan", "silentend", fallback="10:00"), "%H:%M"
        ).time()
        conf["fan"]["interval"] = cfg.getint("fan", "interval")
        # key
        ...
        conf["general"]["debug"] = cfg.getboolean("general", "debug")
        # mqtt
        conf["mqtt"]["host"] = cfg.get("mqtt", "host", fallback="")
        conf["mqtt"]["port"] = cfg.getint("mqtt", "port")
        conf["mqtt"]["username"] = cfg.get("mqtt", "username")
        conf["mqtt"]["password"] = cfg.get("mqtt", "password")
    except Exception:
        # fan
        conf["fan"]["lv0"] = 35
        conf["fan"]["lv1"] = 40
        conf["fan"]["lv2"] = 45
        conf["fan"]["lv3"] = 50
        conf["fan"]["silentmaxlv"] = 0.4
        conf["fan"]["silentstart"] = "22:00"
        conf["fan"]["silentend"] = "10:00"
        conf["fan"]["interval"] = 60
        # key
